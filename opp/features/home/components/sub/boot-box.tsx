...
function MonitorIcon() {
  return <svg className="w-3.5 h-3.5 shrink-0" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="1.5" y="2" width="13" height="9" rx="1.5" /><path d="M5 14h6M8 11v3" /></svg>;
}
function BuildingIcon() {
  return <svg className="w-3.5 h-3.5 shrink-0" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="1.5" y="6" width="13" height="8.5" rx="1" /><path d="M5 14.5V10h6v4.5M1.5 6l6.5-4.5L14.5 6" /></svg>;
}
function SplitIcon() {
  return <svg className="w-3.5 h-3.5 shrink-0" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M8 2v12M4 6l4-4 4 4M4 10l4 4 4-4" /></svg>;
}
function ClockIcon() {
  return <svg className="w-3 h-3 shrink-0" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"><circle cx="8" cy="8" r="6.5" /><path d="M8 4.5V8l2.5 2" /></svg>;
}
function CalendarIcon() {
  return <svg className="w-3 h-3 shrink-0" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="1.5" y="3" width="13" height="11.5" rx="1.5" /><path d="M1.5 7h13M5 1.5V4M11 1.5V4" /></svg>;
}
function TagIcon() {
  return <svg className="w-3 h-3 shrink-0" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M1.5 8.5V3A1.5 1.5 0 0 1 3 1.5h5.5l6 6a1.5 1.5 0 0 1 0 2.1l-5.4 5.4a1.5 1.5 0 0 1-2.1 0l-5.5-5.5z" /><circle cx="5" cy="5" r="1" fill="currentColor" stroke="none" /></svg>;
}
...
function CoverPlaceholder({ label }: { label: string }) {
  return (
    <Box as="div" className="w-full h-full bg-linear-to-br from-sky-50 to-cyan-100 flex flex-col items-center justify-center gap-1">
  ...
  return (
    ...
    >
      ...
      <Box as="div" className="flex items-center gap-2 px-4 pt-3 pb-2 flex-wrap">
        {providerIcon ? (
          <Box as="img" src={providerIcon} alt={provider} className="w-6 h-6 rounded object-cover shrink-0" />
        ) : (
          <Box as="div" className="w-6 h-6 rounded bg-sky-100 flex items-center justify-center text-xs font-bold text-sky-600 shrink-0">
            {provider?.[0] ?? "?"}
          </Box>
        )}
        <Box as="span" className="text-sm font-medium text-gray-700 truncate min-w-0 flex-1">{provider}</Box>
        <Box as="div" className="flex items-center gap-2 ml-auto shrink-0">
          <Box as="span" className={`inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-full ${s.bg} ${s.text}`}>
            <Box as="span" className={`w-1.5 h-1.5 rounded-full ${s.dot} animate-pulse shrink-0`} />
            {s.label}
          </Box>
          {price && (
            <Box as="span" className={`text-xs font-bold shrink-0 ${isFree ? "text-emerald-600" : "text-gray-700"}`}>
              {price}
            </Box>
          )}
        </Box>
  ...
  return (
    ...
    >
      ...
      <Box as="div" className="flex items-start gap-3 sm:gap-4 px-3 sm:px-4 pb-4">
        <Box as="div" className="w-20 h-16 sm:w-28 sm:h-20 rounded-xl overflow-hidden shrink-0 bg-gray-100">
          {coverImage ? (
            <Box as="img" src={coverImage} alt={title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
          ) : (
            <CoverPlaceholder label={provider} />
        ...
        <Box as="div" className="flex flex-col justify-between min-h-16 sm:min-h-20 py-0.5 min-w-0 flex-1">
